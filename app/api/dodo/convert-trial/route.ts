...
export async function POST() {
  ...
  if (!sub?.dodo_subscription_id || sub.dodo_subscription_id.startsWith("checkout-return-")) {
    return NextResponse.json({ error: "No active trial to unlock. Start a plan first." }, { status: 400 });
  }
  ...
  // the app switches from trial limits (3 / 5) to the full plan limits immediately.
  await admin
    .from("subscriptions")
    .update({
      status: "active",
