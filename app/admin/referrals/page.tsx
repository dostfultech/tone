...
export default async function AdminReferralsPage() {
  ...
  for (const r of rows) {
    ...
    const a = agg.get(ref) ?? { signups: 0, subscribed: 0 };
    a.signups += 1;
    if (subscribedSet.has(r.id as string)) a.subscribed += 1;
    agg.set(ref, a);
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8 mt-6 grid gap-4 sm:grid-cols-3">
          <StatCard label="Active referrers" value={String(leaderboard.length)} />
          <StatCard label="Referred signups" value={String(totalSignups)} />
          <StatCard label="Referred subscribers" value={String(totalSubscribed)} highlight={totalSubscribed > 0} />
        </div>
              ...
              {leaderboard.length === 0 ? (
                ...
                leaderboard.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-emerald-600">{row.subscribed}</td>
                  </tr>
                ))
              )}
            </tbody>
