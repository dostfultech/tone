...
export function AccountView() {
  ...
  const statusLabel = snapshot?.hasAccess
    ? formatSubscriptionStatus(snapshot.status)
    : snapshot?.user
      ? "Free access"
      : "Inactive";

  const freeAdaptationsValue = snapshot?.adaptationAccess.isUnlimited
    ? "Unlimited"
    : snapshot?.hasAccess
      ? "Active plan"
            ...
            <section className="compact-card overflow-hidden">
              ...
              <div className="grid gap-5 p-6 xl:grid-cols-[1.1fr_0.9fr]">
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <MetricCard label="Free adaptations" value={freeAdaptationsValue} />
